import { getHashMapPseudocodeState } from './pseudocode'
import type { HashMapOperation, HashMapStep } from './types'

interface HashMapPseudocodePanelProps {
...
function opTitle(op: HashMapOperation | null): string {
...
export default function HashMapPseudocodePanel({
  operation,
  currentStep,
  stepIndex,
  steps,
  ...
  canNext,
  onReset,
}: HashMapPseudocodePanelProps) {
  const { lines, activeLineIndices, executedLineIndices, description } = getHashMapPseudocodeState(
    operation,
    currentStep,
    stepIndex,
    steps
  ...
  if (lines.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-slate-100/80 p-4 dark:border-slate-800 dark:bg-slate-950/40">
        ...
        <p className="text-slate-500 text-sm dark:text-slate-400">Run an operation to see hash map pseudocode step by step.</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-100/80 p-4 flex flex-col gap-3 dark:border-slate-800 dark:bg-slate-950/40">
      <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-100">{opTitle(operation)}</h3>
      {description && (
        <p className="text-sm text-slate-700 bg-rose-50/80 border border-rose-200/80 rounded-lg px-3 py-2 dark:text-slate-100 dark:bg-rose-950/30 dark:border-rose-900/50">
          {description}
        </p>
      )}
