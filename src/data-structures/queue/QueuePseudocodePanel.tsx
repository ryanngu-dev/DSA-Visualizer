...
import { getQueuePseudocodeState } from './pseudocode'

interface QueuePseudocodePanelProps {
...
function operationTitle(op: QueueOperation | null): string {
  if (!op) return 'Pseudocode'
  if (op.type === 'create') return 'Create'
  if (op.type === 'enqueue') return 'Enqueue'
  if (op.type === 'dequeue') return 'Dequeue'
  if (op.type === 'peek') return 'Peek'
  return 'Clear'
}

export default function QueuePseudocodePanel({
  operation,
  currentStep,
  stepIndex,
  steps,
  ...
  canNext,
  onReset,
}: QueuePseudocodePanelProps) {
  const { lines, activeLineIndices, executedLineIndices, description } = getQueuePseudocodeState(
    operation,
    currentStep,
    stepIndex,
    steps
  ...
  if (lines.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-slate-100/80 p-4 dark:border-slate-800 dark:bg-slate-950/40">
        ...
        <p className="text-slate-500 text-sm dark:text-slate-400">Run an operation to see queue pseudocode step by step.</p>
      </div>
    )
  }
