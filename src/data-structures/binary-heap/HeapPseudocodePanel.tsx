import { getHeapPseudocodeState } from './pseudocode'
import type { HeapStep } from './types'
import type { HeapOperation } from './types'

interface HeapPseudocodePanelProps {
...
function operationTitle(op: HeapOperation | null): string {
  if (!op) return 'Pseudocode'
  if (op.type === 'insert') return 'Insert (bubble up)'
  if (op.type === 'extract') {
    return op.mode === 'min' ? 'Extract minimum' : 'Extract maximum'
  }
  if (op.type === 'delete') return 'Delete by value'
  return 'Pseudocode'
}

export default function HeapPseudocodePanel({
  operation,
  currentStep,
  stepIndex,
  steps,
  ...
  canNext,
  onReset,
}: HeapPseudocodePanelProps) {
  const { lines, activeLineIndices, executedLineIndices, description } = getHeapPseudocodeState(
    operation,
    currentStep,
    stepIndex,
    steps
  ...
  if (lines.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-slate-100/80 p-4 dark:border-slate-800 dark:bg-slate-950/40">
        ...
        <p className="text-slate-500 text-sm dark:text-slate-400">Run an operation to see pseudocode step by step.</p>
      </div>
    )
  }
