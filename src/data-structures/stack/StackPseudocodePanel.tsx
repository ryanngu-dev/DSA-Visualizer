...
import { getStackPseudocodeState } from './pseudocode'

interface StackPseudocodePanelProps {
...
function operationTitle(op: StackOperation | null): string {
  if (!op) return 'Pseudocode'
  if (op.type === 'create') return 'Create'
  if (op.type === 'push') return 'Push'
  if (op.type === 'pop') return 'Pop'
  if (op.type === 'peek') return 'Peek'
  return 'Clear'
}

export default function StackPseudocodePanel({
  operation,
  currentStep,
  stepIndex,
  steps,
  ...
  canNext,
  onReset,
}: StackPseudocodePanelProps) {
  const { lines, activeLineIndices, executedLineIndices, description } = getStackPseudocodeState(
    operation,
    currentStep,
    stepIndex,
    steps
  ...
  if (lines.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-slate-100/80 p-4 dark:border-slate-800 dark:bg-slate-950/40">
        ...
        <p className="text-slate-500 text-sm dark:text-slate-400">Run an operation to see stack pseudocode step by step.</p>
      </div>
    )
  }
